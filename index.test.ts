...
import type { UcoPriceResponse } from './index.js';
...
describe('getArchethicUcoPrice', () => {
    ...
    it('should return UCO prices when the API call is successful', async () => {
        const mockApiResponse: UcoPriceResponse = {
            data: {
                oracleData: {
        ...
        } as unknown as Response); // Cast via unknown

        const result = await getArchethicUcoPrice<UcoPriceResponse>();
    ...
    it('should return null if the API response is not ok', async () => {
        ...
        await expect(getArchethicUcoPrice<UcoPriceResponse>()).resolves.toBeNull();
    ...
    it('should return the invalid structure if the response structure is invalid but fetch/json are ok', async () => {
        ...
        const mockInvalidResponse = { // Missing 'data' key entirely
            someOtherKey: 'value'
        };
        ...
        await expect(getArchethicUcoPrice<any>()).resolves.toEqual(mockInvalidResponse);
